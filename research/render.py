...
<p class="note"><strong>A live endpoint does not mean any agent can buy.</strong> Probing
further: <code>tools/list</code> answers anonymously, but every tool <em>call</em> is refused
without a published UCP agent profile URI (<code>invalid_profile_url</code>), verified on three
independent stores. Capability enumeration is open; transacting is not. So these stores are
reachable by <em>identified</em> agents, not anonymous ones — an accountability layer, and a
